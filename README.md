...
- **وضعا القراءة**: القارئ يدعم الآن وضعين قابلين للتبديل من زر في الشريط العلوي: عمودي (تمرير متواصل، الافتراضي) وأفقي (صفحة بصفحة مع تكبير/تصغير عبر photo_view).
- **تحميل مسبق للصور**: يُحمَّل عدد من الصور حول موضع القراءة الحالي مسبقًا في الخلفية (`precacheImage`) بدل انتظار وصول المستخدم لكل صورة، لتجربة تمرير سلسة بلا تأخير ملحوظ.
- **شريط تنقل سفلي**: أُضيف `MainShell` (`lib/screens/main_shell.dart`) يستضيف تبويبي "الرئيسية" و"المفضلة" عبر `IndexedStack` (يحافظ على حالة كل تبويب)، مع أنيميشن خفيف عند التبديل. يظهر هذا الشريط فقط في الشاشة الرئيسية ويختفي تلقائيًا عند فتح تفاصيل عمل أو القارئ (لأنها تُفتح بـ Navigator.push فتغطي الشاشة بالكامل).
